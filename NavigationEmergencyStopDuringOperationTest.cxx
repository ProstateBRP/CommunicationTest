...
NavigationEmergencyStopDuringOperationTest::ErrorPointType NavigationEmergencyStopDuringOperationTest::Test()
{
  ...
  // TODO: How can we differenciate Error(3,3) and Error(3,4)?
  ...
  int fEmergencyStatusTransitioned = 0;
  int fEmergencyStatusConfirmed = 0;
  int fEmergencyCommandSent = 0;
  ...
  for (;;) // Can receive more than 1 transform message
    ...
    if (strcmp(headerMsg->GetDeviceType(), "TRANSFORM") == 0)
      ...
      if (strcmp(headerMsg->GetDeviceName(), "CURRENT_STATUS") == 0)
        {
        if (!CheckAndReceiveStatusMessage(headerMsg, "CURRENT_STATUS", 1, 0, "EMERGENCY"))
          {
          return Error(6,2);
          }
        else
          {
          fEmergencyStatusTransitioned = 1;
          }
        }
      else
        {
  ...
  if (!fEmergencyStatusConfirmed) return Error(6,3);
